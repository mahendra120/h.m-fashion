...
export function HomeClient({
  ...
  return (
    <PublicLayout>
      ...
      {/* ===== Marquee value strip ===== */}
      <section className="border-y bg-primary py-3 text-primary-foreground">
        ...
        <StaggerGroup className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5 sm:gap-4">
          {categories.map((cat, i) => (
            <StaggerItem key={cat.id}>
              <CategoryTile cat={cat} large={i === 0} />
            </StaggerItem>
          ))}
        </StaggerGroup>
      </section>
...
function CategoryTile({ cat, large }: { cat: Category; large?: boolean }) {
  return (
    <Link
      href={`/shop?cat=${cat.slug}`}
      className={`group relative block overflow-hidden rounded-2xl bg-muted ${large ? 'aspect-[4/5] sm:aspect-[4/3] lg:aspect-[3/5]' : 'aspect-[4/5]'}`}
    >
      <Image
        src={cat.image}
        alt={cat.name}
